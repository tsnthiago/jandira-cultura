  ...
  const handleSave = async (newPoint: any) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(
          savedPoint.message || 'Erro ao adicionar o ponto turístico'
        );
      }
    } catch (error) {
  ...
  return (
    <div>
      <main className="container mx-auto p-8">
        {' '}
        {/* Adiciona um container para melhor layout */}
        ...
        <div className="mt-8">
          {points.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {points.map((point, index) => (
                <div key={point._id} className="p-4 border rounded shadow">
                  ...
                  <p>
                    <strong>Tags:</strong> {point.tags.join(', ')}
                  </p>
                </div>
              ))}
            </div>
            ...
            <p className="text-center">
              Nenhum ponto turístico adicionado ainda.
            </p>
          )}
        </div>
      </main>
